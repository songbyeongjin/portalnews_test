...
def handler(event, context):
    ...
    for index in range(0, NEWS_LEN):
        ...
        insert_press   = daum_crwal_cor[index].text
        insert_wrtier  = "writer"
        insert_url     = daum_crwal_url[index].get('href')#!<urlは予め取得しておいた値を使用する
        insert_portal  = DAUM
...
def get_daum_url_cor(soup):
    urlCor = []
    daum_crwal_url = []
    daum_crwal_cor = []

    daum_crwal_url = soup.select(".list_news2 li a")
    urlCor.append(daum_crwal_url)
    daum_crwal_cor = soup.select(".info_news")
    urlCor.append(daum_crwal_cor)
    
    return urlCor

def get_daum_title(soup):
    daum_crwal_title = soup.select(".tit_view")
    return daum_crwal_title[0].text

def get_daum_content(soup):
    temp_contents = soup.select("#harmonyContainer p")
    trimminged_content = " ".join(temp_contents[0].text)
